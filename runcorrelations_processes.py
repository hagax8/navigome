...
for i in codes:
    ...
    cmd = ['python', 'navigome_vis4_corr.py',
           'correlation_data/'+str(i)+'.correlations', str(i), " %.2e" % (cutoff),
           'correlations/'+str(i)+'.correlations']
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    (output, err) = process.communicate()
    p_status = process.wait()
    print(str(i))
    with open('correlations/'+str(i)+'.correlations'+'.html','r') as f:
        head = newheader + '<title>' + "Genetic correlations: " + str(i) + '</title>'
        html = f.read()
    ...
    with open('correlations/'+str(i)+'.correlations'+'.html','w') as fout:
        fout.write(str(soup))
