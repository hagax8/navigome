...
correlations = pd.read_csv(argv[1])
phencode = argv[2]
...
df['phenotype_reference']=df['phenotype'].astype(str)+' ('+df['code'].astype(str)+') '
phenlink = lambda x: "https://www.ncbi.nlm.nih.gov/pubmed/"+str(x) if (len(str(x).strip()) == 8 and x.isdigit()) else "nopmid.html"
df['phenotype_link'] = df['PMID or link'].apply(phenlink)
fancycap = lambda x: x[0].upper()+x[1:]
