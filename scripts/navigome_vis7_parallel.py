...
data2 = pd.DataFrame([{"ThresholdVal": 4.70813, "Threshold": u"PrediXcan zscore \u2265 4.70813", "Text": "Significance Threshold"}])
data3 = pd.DataFrame([{"ThresholdVal": -4.70813, "Threshold": u"PrediXcan zscore \u2264 -4.70813"}])
highlight = alt.selection(type='single', on='mouseover',
                          fields=['name'], nearest=True)
df['urlgene'] = "https://phenviz.navigome.com/gene_phenotypes/"+df['ENSEMBL'].astype(str)+'.html'

select4 = ['Adipose_Subcutaneous',
           'Adipose_Visceral_Omentum','Breast_Mammary_Tissue',
           'Muscle_Skeletal','Skin_Not_Sun_Exposed_Suprapubic',
           'Skin_Sun_Exposed_Lower_leg','Cells_Transformed_fibroblasts','Cells_EBV_transformed_lymphocytes']
...
mycolall=['Adipose_Subcutaneous','Adipose_Visceral_Omentum','Breast_Mammary_Tissue',
             'Muscle_Skeletal','DGN_Whole_Blood','Whole_Blood',
             ...
             'Brain_Caudate','Brain_Nucleus_accumbens',
             'Brain_Putamen','Brain_Cerebellar_Hemisphere',
             'Brain_Cerebellum','Brain_Cortex','Brain_Frontal_Cortex_BA9',
             'Brain_Hippocampus','Brain_Hypothalamus',
             'Brain_Spinal_cord_cervical_C1','Brain_Substantia_nigra',
             'Nerve_Tibial','Cells_Transformed_fibroblasts','Cells_EBV_transformed_lymphocytes',
             'Colon_Sigmoid','Colon_Transverse','Esophagus_Gastroesophageal_Junction',
             'Esophagus_Mucosa','Esophagus_Muscularis','Liver','Minor_Salivary_Gland',
             'Small_Intestine_Terminal_Ileum','Stomach','Adrenal_Gland',
             'Pituitary','Pancreas','Thyroid','Lung','Ovary','Prostate','Testis',
             'Uterus','Vagina','Skin_Not_Sun_Exposed_Suprapubic',
...
for mycol in selectall:
    ...
    df_transformed = df.reset_index().melt(['index','name', 'urlgene', 'ENSEMBL','association'])
    ...
    df_transformed.columns=df_transformed.columns.str.replace('value','predixcan_zscore')
    line=alt.Chart().mark_line().encode(
        detail='index',
        x=alt.X('variable:N',title='',axis=alt.Axis(labelAngle=45)),
        y=alt.Y('predixcan_zscore:Q',title='S-PrediXcan z-score',scale=alt.Scale(domain=(-maxall,maxall))),
        tooltip=['name','ENSEMBL','association','predixcan_zscore'],
        color=alt.condition(~highlight,alt.value('grey'),alt.value('black'))
    )
    ...
    chartcompound=alt.layer(line1,line2, annotation,rules1,rules2,data=df_transformed).properties(width=500,height=250,title=title)
