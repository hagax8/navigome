...
df['Phenotype'] = df['phenotype'].apply(
    lambda x: x[:50] + '...' if len(x) > 50 else x)
df['Phenotype_code'] = df['Phenotype'].astype(
    str) + ' (' + df['code'].astype(str) + ') '

select_box_category = alt.binding_select(options=list(
    np.sort(np.asarray(df['Phenotype'].unique()))))
