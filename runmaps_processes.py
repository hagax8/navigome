...
f_reference = "gwas_reference"

# corresponding correlations
f_correlations = "correlation_matrix"
...
cmd = ['python', 'dimred.py', f_correlations, outdir+'/'+out_maps]
...
for i in output_umap:
    cmd = ['python', 'navigome_vis3_map.py', outdir+'/'+i, 'UMAP', outdir+'/'+i]
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    print('UMAP...')
    for line in process.stdout:
        print(line)
    print('done')
for i in output_umap:
    cmd = ['python', 'navigome_vis3_map_mobile.py', outdir+'/'+i, 'UMAP',
           outdir+'/'+i+"_mobi"]
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    print('UMAP...')
    for line in process.stdout:
...
for i in output_gtm:
    cmd = ['python', 'navigome_vis3_map.py', outdir+'/'+i, 'GTM', outdir+'/'+i]
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    print('GTM...')
    for line in process.stdout:
        print(line)
    print('done')
for i in output_gtm:
    cmd = ['python', 'navigome_vis3_map_mobile.py', outdir+'/'+i,
           'GTM', outdir+'/'+i+"_mobi"]
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    print('GTM...')
    for line in process.stdout:
...
for i in output_tsne:
    cmd = ['python', 'navigome_vis3_map.py', outdir+'/'+i,
           't-SNE', outdir+'/'+i]
    process = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    print('t-SNE...')
    for line in process.stdout:
...
navbar1 = ('<nav class="navbar navbar-default align-middle" role="navigation" style="color:grey;font-family: \'Open Sans\', sans-serif;font-size: 18px;font-weight:300">' +
          ...
          '</ul>' +
          '</nav>')
navbar2 = ('<nav class="navbar navbar-default" role="navigation" style="color:grey">' +
          '<ul class="nav nav-tabs bg-dark">' +
